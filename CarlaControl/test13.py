...
# 设置汽车的初始位置和朝向
transform1 = carla.Transform(carla.Location(x=-26.542, y=135.5, z=0.5), carla.Rotation(yaw=0))
transform2 = carla.Transform(carla.Location(x=-5.5, y=120.5, z=0.5), carla.Rotation(yaw=90))## 45 130
transform3 = carla.Transform(carla.Location(x=-20, y=130, z=0.5), carla.Rotation(yaw=-180))  ##20 127
# 设置行人的初始位置和朝向
walker_transform1 = carla.Transform(carla.Location(x=-21.5, y=132.5, z=0.5), carla.Rotation(yaw=-90))
walker_transform2 = carla.Transform(carla.Location(x=-21.5, y=138.5, z=0.5), carla.Rotation(yaw=-90))
# 在世界中生成三辆汽车
...
walker_controller1.go_to_location(carla.Location(x=-21.5, y=140, z=0.500000))  # 设置行人目的地
walker_controller1.set_max_speed(0.5)  # Between 1 and 2 m/s (default is 1.4 m/s).
# 开启控制
walker_controller2.start()
walker_controller2.go_to_location(carla.Location(x=-21.5, y=120, z=0.500000))  # 设置行人目的地
...
while True:
    ...
    spectator.set_transform(carla.Transform(transform1.location + carla.Location(z=20.565),
                                            carla.Rotation(pitch=-90)))  # 设置跟随汽车视角
    if get_traffic_light_color(vehicle1) == "Red":
        set_traffic_light_color(vehicle1.get_traffic_light(),"Green")
    # print(get_traffic_light_color(vehicle1))
